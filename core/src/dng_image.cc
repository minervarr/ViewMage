...
// Everything the develop needs about the file, parsed once and shared by
// decode_dng and read_raw_mosaic. `raw_off` locates the single strip.
struct RawParse {
    RawMeta m;
    size_t raw_off = 0;
    bool ok = false;
...
RawParse parse_raw(const uint8_t* data, size_t size, NoiseModel* out_noise) {
    RawParse out;
    auto fail = [&out](const char* why) { out.error = why; return out; };
...
RawMosaicView read_raw_mosaic(const uint8_t* data, size_t size,
                              NoiseModel* out_noise) {
    RawMosaicView view;
    if (!looks_like_dng(data, size)) {
        view.error = "Not a DNG or TIFF file.";
        return view;
    }
    const RawParse raw = parse_raw(data, size, out_noise);
    if (!raw.ok) {
        view.error = raw.error;
        return view;
    }
    view.bayer = reinterpret_cast<const uint16_t*>(data + raw.raw_off);
    view.width  = uint32_t(raw.m.width);
    view.height = uint32_t(raw.m.height);
    view.cfa    = raw.m.cfa;
    for (int i = 0; i < 4; ++i) view.black[i] = raw.m.black[i];
    view.white       = raw.m.white;
    view.has_neutral = raw.m.has_neutral;
    for (int i = 0; i < 3; ++i) view.neutral[i] = raw.m.neutral[i];
    view.baseline_exposure = raw.m.baseline_exposure;
    view.ok = true;
    return view;
}
...
DecodedImage decode_dng(const uint8_t* data, size_t size, uint32_t maxDimension,
    ...
    const RawParse raw = parse_raw(data, size, out_noise);
    if (!raw.ok) return fail(raw.error.c_str());
    const RawMeta& m = raw.m;

    const int W = m.width;
    const int H = m.height;

    // ── Develop ─────────────────────────────────────────────────────────────
    const uint16_t* bayer = reinterpret_cast<const uint16_t*>(data + raw.raw_off);
    ...
    // Luma weights for the working gamut, used by highlight fringe suppression.
    // BT.709's weights on BT.2020 data skew the luminance per hue — the same
    // green-cast mistake the export and enhance fixed at their own layers.
    const bool wide_working = (primaries == WorkingPrimaries::Bt2100);
    const float lw_r = wide_working ? 0.2627f : 0.2126f;
    const float lw_g = wide_working ? 0.6780f : 0.7152f;
    const float lw_b = wide_working ? 0.0593f : 0.0722f;
    ...
    // ── Detail layer (AI path) ──────────────────────────────────────────────
    //
    // RawNIND smooths sub-3px structure -- measured, single-pass keeps only
    // ~71% of Malvar's fine-texture energy, and the user reads that as a
    // resolution LOSS even at identical pixel counts. The layer below returns
    // the develop's own measured micro-detail: the Laplacian of the MALVAR
    // luma (pure sensor measurement, nothing invented) is scaled multiplicat-
    // ively onto the AI base. Hue is preserved exactly -- every channel is
    // multiplied by the same factor -- and gain 1.0 returns every bit of
    // measured acuity (DETAIL-FIRST, chosen deliberately; expect some sensor
    // grain back in deep shadow).
    //
    // Implemented as a deferred row pipeline: a pixel's horizontal blur
    // neighbourhood is complete once its row ends, but the vertical ±2 rows
    // are not until two rows later -- so each completed row is emitted two
    // rows behind the writer, drawing lmM from a 3-row ring.
    //
    // GAIN PER MODE. Single-pass keeps only ~71% of Malvar's structure, so it
    // takes the full 1.0 return. The ensemble used to be left bare — but its
    // disagreement-guided speckle removal (see ai_denoise.cc) now leaves a
    // CONSISTENT base sitting at ~0.45-0.75x of Malvar's fine energy, and the
    // user reads that as softness; a moderate measured return lands between
    // "mush" and yesterday's rejected grain-over-smooth (which stacked on
    // SPURIOUS speckle — a base problem this no longer has). The constant is
    // pinned by --compare patch-grid targets: weak patches ~0.8-1.1x Malvar,
    // textured ~1.0-1.15x.
    const bool want_detail = true;
    const float kDetailReturn = denoiseEnsemble ? 0.55f : 1.0f;
    constexpr int kBlendV = 4;             // matches the border ramp width
    std::vector<float> cur_m(size_t(W), 0.f);     // Malvar luma, row in build
    std::vector<float> cur_a(size_t(W), 0.f);     // blended luma, row in build
    std::vector<unsigned char> cur_ok(size_t(W), 0); // full-AI per pixel
    std::vector<float> m_ring(size_t(W) * 4, 0.f); // completed rows, mod 4
    std::vector<float> a_ring(size_t(W) * 4, 0.f);
    std::vector<unsigned char> ok_ring(size_t(W) * 4, 0);
    int rows_done = 0;
    long detail_emits = 0;
    double detail_s_sum = 0.0;

    // Emit row ry: multiply pixel luma by 1 + H/lumAI where H is Malvar's
    // local detail against the completed neighbourhood (5x3 box). Only pixels
    // whose own coverage was FULLY AI are touched -- the border band stays
    // exactly what the blend produced. Hue is preserved because every channel
    // carries the same factor; negative out-of-gamut components survive.
    auto detail_emit = [&](int ry) {
        float* dst = &rgba[size_t(ry) * W * 4];
        const unsigned char* okr = &ok_ring[size_t(((ry % 4) + 4) % 4) * W];
        for (int x = 0; x < W; ++x) {
            if (!okr[x]) continue;
            const double lmC = m_ring[size_t(((ry % 4) + 4) % 4) * W + x];
            double hb = 0.0;
            for (int dy = -1; dy <= 1; ++dy)
                for (int dx = -2; dx <= 2; ++dx) {
                    const int xx = std::min(std::max(x + dx, 0), W - 1);
                    hb += m_ring[size_t((((ry + dy) % 4) + 4) % 4) * W + xx];
                }
            const double lmB = hb / 15.0;              // 5x3 box
            const float H   = float(lmC - lmB);        // measured micro-detail
            const float LA  = a_ring[size_t(((ry % 4) + 4) % 4) * W + x];
            // Floor guards tiny denoms in deep black; clamp keeps a single
            // pixel spike from producing a runaway factor on specular edges.
            float s = 1.0f + kDetailReturn * H / std::max(LA, 5e-3f);
            s = std::min(std::max(s, 0.25f), 4.0f);
            ++detail_emits;
            detail_s_sum += s;
            float* p = &dst[size_t(x) * 4];
            p[0] *= s; p[1] *= s; p[2] *= s;
        }
    };
    ...
    // Which photosites were clipped BEFORE the pack's [0,1] clamp erased the
    // fact. The model sees a ceiling-flat core and invents colour around it
    // (measured: B/G 2.9 -> 4.2 on a cyan LED, R/G 1.46 -> 0.95 with channel
    // collapse on an orange one); this mask is the ground truth the develop
    // reconstructs from instead.
    std::vector<uint8_t> ai_clip;
    ...
    if (denoiser) {
        ...
        pack_bayer_rggb(bayer, W, H, stride_px, m.cfa, eff_black, m.white,
                        packed, pw, ph, ai_ox, ai_oy, &ai_clip);
        std::string derr;
    ...
    for (int y = 0; y < H; ++y) {
        for (int x = 0; x < W; ++x) {
            ...
            // Compute Malvar demosaic for this pixel (always, for blending).
            {
            ...
            // Malvar luma of this pixel, captured before the blend replaces
            // it -- the detail layer measures structure HERE.
            const float mv_r = rgb[0], mv_g = rgb[1], mv_b = rgb[2];

            // Soft blend at the AI/Malvar boundary: smoothstep over 4 pixels
            // instead of a hard 0/1 switch.  The AI result is higher quality
            // everywhere it covers; Malvar fills the 0-1 pixel border left by
            // the RGGB phase shift.  Blending eliminates any visible edge.
            if (ai && ax >= 0 && ay >= 0 && ax < ai_w && ay < ai_h) {
                float w_ai = 1.0f;
                if (ax < kBlendV)
                    w_ai = float(ax + 1) / float(kBlendV + 1);
                else if (ax >= ai_w - kBlendV)
                    w_ai = float(ai_w - ax) / float(kBlendV + 1);
                if (ay < kBlendV)
                    w_ai = std::min(w_ai, float(ay + 1) / float(kBlendV + 1));
                else if (ay >= ai_h - kBlendV)
                    w_ai = std::min(w_ai, float(ai_h - ay) / float(kBlendV + 1));
                // Smoothstep for a perceptually uniform transition.
                w_ai = w_ai * w_ai * (3.0f - 2.0f * w_ai);

                const float* s = &ai[(size_t(ay) * ai_w + ax) * 3];
                rgb[0] = w_ai * s[0] + (1.0f - w_ai) * rgb[0];
                rgb[1] = w_ai * s[1] + (1.0f - w_ai) * rgb[1];
                rgb[2] = w_ai * s[2] + (1.0f - w_ai) * rgb[2];
            }
            // Fully-AI pixel (border ramp never touched it): record both the
            // pre-blend Malvar luma and the blended luma for this row. The
            // deferred detail emitter two rows behind uses them.
            const bool full_ai =
                ai && ax >= kBlendV && ay >= kBlendV &&
                ax < ai_w - kBlendV && ay < ai_h - kBlendV;
            cur_ok[size_t(x)] = full_ai ? 1 : 0;
            cur_m[size_t(x)] = lw_r * mv_r + lw_g * mv_g + lw_b * mv_b;
            cur_a[size_t(x)] = lw_r * rgb[0] + lw_g * rgb[1] + lw_b * rgb[2];


            float rr = rgb[0]*g[0], gg = rgb[1]*g[1], bb = rgb[2]*g[2];

            // ── Mosaic-guided highlight handling (AI path only) ─────────────
            //
            // ai_clip knows which photosites measured at or above white before
            // the pack's clamp flattened them to the ceiling. The model, fed
            // that flat core, invents ratios there: measured on a night frame,
            // a cyan LED's B/G went 2.9 -> 4.2 and an orange lamp's core went
            // R/G 1.46 -> 0.95 with B collapsing to 0.02. This block restores
            // what the clamp discarded, working POST white balance so a fully
            // clipped core lands on equal channels -- display-neutral white --
            // rather than the camera's pre-WB variant, which leaves its gains
            // in the result.
            if (!ai_clip.empty() && ax >= 0 && ay >= 0 && ax < ai_w && ay < ai_h) {
                // Clip fractions per output channel from the 3x3 cell
                // neighbourhood: 1.0 = every photosite of that colour clipped,
                // 0 = none did. The neighbourhood softens the mask edge so the
                // correction ramps in over ~2 px instead of ringing.
                // Cells are indexed in AI-plane space (ax,ay): the pack cropped
                // ai_ox/ai_oy off the mosaic, so image-space division by two
                // would drift by one cell whenever that crop is non-zero.
                const int cx = ax >> 1;
                const int cy = ay >> 1;
                const int cw = ai_w >> 1, ch = ai_h >> 1;
                double fr[3] = {0, 0, 0};
                int fn[3] = {0, 0, 0};
                for (int dy = -1; dy <= 1; ++dy)
                    for (int dx = -1; dx <= 1; ++dx) {
                        ...
                        const uint8_t bits = ai_clip[size_t(ny) * cw + nx];
                        if (!bits) continue;
                        for (int i = 0; i < 4; ++i)
                            if (bits & (1u << i)) {
                                // Packed planes after the RGGB force are
                                // [R, G1, G2, B]; both greens feed channel 1.
                                const int cc = (i >= 2) ? i - 1 : i;
                                fr[cc] += 1.0; ++fn[cc];
                            }
                    }
                for (int c = 0; c < 3; ++c) if (fn[c]) fr[c] /= fn[c];

                const float fmax = std::max(fr[0], std::max(fr[1], fr[2]));
                if (fmax > 0.0f) {
                    auto sstep = [](float t) {
                    ...
                    float v[3] = {rr, gg, bb};
                    // Raise each clipped channel to the brightest channel that
                    // did NOT clip (camera rule: clipped channels rise, honest
                    // ones keep their value). A fully-clipped core therefore
                    // becomes exactly neutral.
                    for (int c = 0; c < 3; ++c) {
                        if (!(fr[c] > 0.0f)) continue;
                        float peak = 0.0f;
                        bool have = false;
                        for (int k = 0; k < 3; ++k)
                            if (fr[k] < 0.01f && (!have || v[k] > peak)) {
                                peak = v[k]; have = true;
                            }
                        if (!have)
                            peak = std::max(v[0], std::max(v[1], v[2]));
                        v[c] += (peak - v[c]) * sstep(float(fr[c]));
                    }
                    // Fringe suppression: where clipping is present but partial,
                    // the model's invented hue is pulled toward the pixel's own
                    // luminance -- chroma dies exactly where it cannot be
                    // trusted, and untouched pixels are unaffected.
                    const float Ys = lw_r * v[0] + lw_g * v[1] + lw_b * v[2];
                    constexpr float kFringeSuppress = 0.65f;
                    const float hs = kFringeSuppress * sstep(fmax);
                    for (int c = 0; c < 3; ++c) v[c] += (Ys - v[c]) * hs;
        ...
        // Row complete: emit the row that gained its full vertical
        // neighbourhood BEFORE storing this one (a mod-4 ring holds four past
        // rows, exactly the window the emitter reads), then file it.
        if (ai && want_detail) {
            if (rows_done >= 3) detail_emit(y - 2);
            const size_t slot = size_t(((y % 4) + 4) % 4);
            std::memcpy(&m_ring[slot * W], cur_m.data(), sizeof(float) * size_t(W));
            std::memcpy(&a_ring[slot * W], cur_a.data(), sizeof(float) * size_t(W));
            std::memcpy(&ok_ring[slot * W], cur_ok.data(), sizeof(char) * size_t(W));
            ++rows_done;
        }
    }

    if (ai && std::getenv("VIEWMAGE_DETAIL_DEBUG"))
        std::fprintf(stderr, "detail layer: rows=%d emitted=%ld mean s=%.4f\n",
                     rows_done, detail_emits,
                     detail_emits ? detail_s_sum / double(detail_emits) : 1.0);
